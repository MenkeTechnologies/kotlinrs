...
const CORPUS: &[Entry] = &[
    ...
    (
        ...
        "Operators",
        "a == b",
        "Structural equality. It compares numbers by value across `Int`/`Double`, `List`s element-wise, `Set`s order-insensitively, `Map`s entry-wise, and data-class instances over their primary-constructor properties. An `Array` inherits identity equality, so `arrayOf(1) == arrayOf(1)` is `false`. Contrast `===`.",
    ...
    (
        ...
        "The entries as `Pair`s, in iteration order. The pairs print `(k, v)` where the entries themselves print `k=v`, which is the whole reason this is not the generic sequence `toList`.",
