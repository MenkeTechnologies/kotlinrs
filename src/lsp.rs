...
const CORPUS: &[Entry] = &[
    ...
    (
        ...
        "Types",
        "Int",
        "Signed integer. It is 64-bit at run time — kotlinrs has one integer representation, so `Int` and `Long` are the same value and `Int` arithmetic does not overflow at 32 bits the way the JVM's does. `/` and `%` truncate toward zero.",
        "val n: Int = 7 / 2\nprintln(n)   // 3",
    ),
    (
        "Long",
        "Types",
        "Long",
        "Signed 64-bit integer, sharing `Int`'s representation and division rules. An `L` literal suffix is accepted and prints without it, so `println(10L)` writes `10`.",
        "val big: Long = 10L\nprintln(big)   // 10",
    ),
    (
        "Double",
    ...
    (
        ...
        "Numeric Members",
        "Int.toInt(): Int\nDouble.toInt(): Int",
        "Truncates a `Double` toward zero; on an integer it is the identity. There is no `String.toInt` — parsing text to a number is not implemented.",
        "println(3.9.toInt())   // 3",
    ),
    (
        "toLong",
        "Numeric Members",
        "Int.toLong(): Long\nDouble.toLong(): Long",
        "The same conversion `toInt` performs, since `Int` and `Long` share one 64-bit representation here.",
    ...
    (
        ...
        "Yields the receiver when the predicate returns `true`, and `null` otherwise. `takeUnless` — its negation — is not implemented.",
